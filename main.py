...
def update_packages(updates: Dictionary[str, Any], location: Location, 
                    packages: List[Package], timestamp: datetime, truck_id: int):
...
def calc_piority(distance: float, avg_distance: float, 
                      earliest_arrival_time: datetime, cur_time: datetime):
    ...
    if(est_arrival_time < earliest_arrival_time):
        est_arrival_time = earliest_arrival_time
    
    travel_seconds = (est_arrival_time - cur_time).total_seconds()
    
    # Make priority a balance between meeting deadlines and travel distance.
    # Lower numbers are higher priority.
    priority = distance-(avg_distance/travel_seconds)
...
def route_load(start_location_id: int, start_time: datetime, truck_id: int, 
               load: List[int], matrix: List[List[float]], 
...
def plan_truck_schedule(schedule: List[List[int]], 
    ...
    while 0 < len(schedule):
        path, cost = route_load(last_location, start_time, schedule.pop(), matrix)
        routes.append(path)
...
route2, end_time2, miles2 = plan_truck_schedule(truck2, start_of_day, matrix)
